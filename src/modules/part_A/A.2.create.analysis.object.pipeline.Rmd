...
baseDesignFN <- Obio@projectDetailList$baseDesignFN

if (file.exists(baseDesignFN)){
...
rsemFN <- Obio@projectDetailList$RSEMcountDataFile
...
dfRename <- unique(
    dfDesign[,c("original.sample.id", "sample.id")]
)

rownames(dfRename) <- dfDesign$original.sample.id
...
dfTPM <- dfRSEM
dfTPM[[Obio@parameterList$primaryAlignmentGeneID]] <- row.names(dfTPM)
dfTPM <- dfTPM[,c(Obio@parameterList$primaryAlignmentGeneID, names(dfRSEM))]
...
modelFN <- "/camp/stp/babs/working/boeings/Projects/boeings/stefan.boeing/474_bulk_RNAseq_pipeline/basedata/biologic/inputs/design.model.file.txt"
