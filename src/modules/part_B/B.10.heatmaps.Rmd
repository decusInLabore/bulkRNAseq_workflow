...
if (!is.null(Ntop4pcaGeneSelection) | 
...
cat.name <- paste0("Experiment_",project_id, "_",Obio@parameterList$NtopGene,"_most_variable_genes")
...
dfSelections[["designColumn"]] <- numextract(dfSelections$padjSelections)
dfSelections[["designColumn"]] <- gsub("contrast_", "comp_", dfSelections[["designColumn"]] )
dfSelections[["designColumn"]] <- gsub("_$", "", dfSelections[["designColumn"]] )
dfSelections$designColumn[!(dfSelections$designColumn %in% names(dfDesign))] <- ""
...
for (k in 1:length(HmDisplayCatsFromDb)){
    ...
    ## Check ##
    sampleSelection <- names(dfMainData)[sapply(sampleSelection, function(x) grep(x, names(dfMainData)))]
    ...
    if (length(geneSel) > 2){
        ...
        HMplotList[[names(HmDisplayCatsFromDb)[k]]] = ComplexHeatmap::Heatmap(
            ...
            column_title = gsub(
                    "_", 
                    " ", 
                    paste0("Heatmap_", names(HmDisplayCatsFromDb)[k])
            ),
            name = paste0("HM_", k), 
            #row_km = 5,
            col = f1,
    ...
    if (! is.null(HmDisplayCatsFromDb[[k]]$cat_id)){
        ...
        link <- paste0("https://biologic.crick.ac.uk/",project_id,"/category-view/",HmDisplayCatsFromDb[[k]]$cat_id)
    } else {
        link <- ""
    }
