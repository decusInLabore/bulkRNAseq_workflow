...
Obio <- biologicSeqTools::createDdsObject(Obio)
...
Obio@databaseTable <- biologicSeqTools::datatable.to.website.ptm(
    df.data = df.summary,
...
cmd.vec <- biologicSeqTools::upload.datatable.to.database(
  host = Obio@dbDetailList$host,
  ...
  db.col.parameter.list = biologicSeqTools::inferDBcategories(
      dfDesign4upload
  ),
...
cmd.vec <- upload.datatable.to.database(
    ...
    db.col.parameter.list = biologicSeqTools::inferDBcategories(Obio@databaseTable[Obio@databaseTable$count_cut_off > 1, ]),
    new.table = TRUE
)
...
biologicSeqTools::create.gsea.rnk.files(
     Obio@parameterList$localWorkDir,
