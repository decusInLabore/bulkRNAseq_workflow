...
if (!is.null(Obio@projectDetailList$DEseq2_DGE_result_folder) && Obio@projectDetailList$DEseq2_DGE_result_folder != ""){
    externalDGEoutputsToAdd <- paste0(
        Obio@projectDetailList$DEseq2_DGE_result_folder,
        list.files(Obio@projectDetailList$DEseq2_DGE_result_folder)
    )
    
    externalContrastName <- list.files(Obio@projectDetailList$DEseq2_DGE_result_folder)
    externalContrastName <- gsub(".txt", "", externalContrastName)
    
    for (i in 1:length(externalDGEoutputsToAdd )){
      
        # Max colname length is 64, including tags
        colName <- substr(externalContrastName[i], 1, 42)
      
        res <- read.delim(
            externalDGEoutputsToAdd[i],
            sep = "\t"
        )
        #res = data.frame(res)
        names(res) = paste(names(res), colName, sep="_")
        ...
        ## Add to result array ##
        if (nrow(Obio@DEseq2contrastTable) == 0){
            Obio@DEseq2contrastTable <- res
        } else {
