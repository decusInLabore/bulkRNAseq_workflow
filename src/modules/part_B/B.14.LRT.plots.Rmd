...
```{r LRT_module, echo=TRUE, eval=TRUE, warning=FALSE}
chnkPrefix <- "C12.LRT."
...
plotList <- list()
lrtVec <- names(dfDesign)[grep("LRT_",names(dfDesign))]
...
for (i in 1:length(lrtVec)){
    ...
    selVec <- c(
      ...
      paste0("contrast_L_lg2BaseMean_", lrtVec[i]),
      paste0("contrast_L_lg10p_", lrtVec[i])
    )
   ...
   link <- paste0("https://biologic.crick.ac.uk/",Obio@parameterList$project_id,"/scatterplot?x_axis=",selVec[2],"&y_axis=",selVec[3],"&cat_id=ag_lab_categories__10")
    ...
    NewChnk <- paste0(
        paste0("### ",tag," \n"),
            "\n```{r LRT_, ",tag,"  , results='asis', echo=F, eval=TRUE, warning=FALSE, fig.cap='",figCap,"'}\n",
            "\n",
            "\n print(plotList[['",tag,"']])",
            "\n cat(  '\n')",
            "\n\n\n```\n"   
...
```{r lrt_plot, echo=TRUE, eval=TRUE, warning=FALSE, results="asis"}
