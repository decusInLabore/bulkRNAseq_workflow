...
```{r partC_C6_sample_characterization_init, echo=TRUE, eval=TRUE, warning=FALSE}
chnkPrefix <- "C6."
...
```{r sample_characterization_data_prep, echo=F, eval=TRUE, warning=FALSE, results=F}
...
if (length(grep("CoVar", names(dfMainData))) > 0){
    ...
    link <- paste0(
              ...
              '<a href="https://',urlString,'/',Obio@parameterList$project_id,'/scatterplot?x_axis=CoVarOrder&y_axis=CoVar&headline=2D+Scatterplot" target="_blank">here</a>', '. ')  
...
```{r sample_characterization_data_plot, echo=TRUE, eval=TRUE, warning=FALSE, results="asis"}
