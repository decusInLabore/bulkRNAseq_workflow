...
```{r partC_C9_pca_init, echo=TRUE, eval=TRUE, warning=FALSE}
chnkPrefix <- "C9.DGE."
...
```{r MA_and_volcano_data_prep, echo=TRUE, eval=TRUE, warning=FALSE, results=F}
...
MAplots <- names(Obio@plotCollection)[grep("MAplot_",names(Obio@plotCollection))]
Vplots <- gsub("MAplot_", "Volcano_Plot_", MAplots)

MAplotList <- list()
VplotList <- list()

## Create dfMAplots ##
contrastSel <- names(dfMainData)[grep("contrast_[0-9]{1,2}", names(dfMainData))]
MAselVec <- c(
...
for (i in 1:length(contrastVec)){
    ...
    yAxis <- selLg2BM[grep(contrastVec[i], selLg10p)]
    
    link2 <- paste0('<a href="https://biologic.crick.ac.uk/',Obio@parameterList$project_id,'/scatterplot?x_axis=',xAxis,'&y_axis=',yAxis,'&cat_id=ag_lab_categories__10" target="_blank">here</a>.')
...
```{r volcano_plot, echo=TRUE, eval=TRUE, warning=FALSE, results="asis"}
