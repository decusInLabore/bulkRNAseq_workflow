...
```{r partC_C11B_pca_init, echo=TRUE, eval=TRUE, warning=FALSE}
chnkPrefix <- "C11.GSE."
...
## Create Excel output ##
fullOutFN <- paste0(Obio@parameterList$reportTableDir, Obio@parameterList$project_id, "_GSEA.xlsx")
...
for (i in 1:length(rnkFileVec)){
   ...
   ###########################################################################
    ...
    sn <- gsub("GSEA", "", substr( paste0(tag, "_GSEA"), 1, 30))
...
```{r plot_top_up_down, echo=TRUE, eval=TRUE, warning=FALSE, results="asis"}
...
```{r plot_GSEA_cat_overview, echo=TRUE, eval=TRUE, warning=FALSE, results="asis"}
