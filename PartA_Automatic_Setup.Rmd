...
for (i in 1:length(files)){
    ...
    dfTemp <- data.frame(readRDS(files[i]))
    write.table(
        dfTemp,
...
externalFiles <- list.files( paste0(pipelineList[["dataDir"]], "DEseq2External/"))

if (length(externalFiles) > 0){
    externalFiles <- gsub(".txt", "", externalFiles)
    for (i in 1:length(externalFiles)){
        dfDesign[[paste0("comp_", externalFiles[i])]] <- ""
    }
}
