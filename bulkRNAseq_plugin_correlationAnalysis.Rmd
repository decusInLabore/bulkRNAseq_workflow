...
library(circlize)
f1 = colorRamp2(seq(-1, 1, length = 3), c("#3060cf", "#fffbbc","#c4463a"))


#dfTop = data.frame(Type = c(rep("Glia", 15),rep("Transition",5),  rep("Neuron", 80)))
#top_ha = HeatmapAnnotation(df = dfTop, col = list(Type = c("Glia" =  "orange", "Transition" = "grey", "Neuron" = "#009900")))

# split
#left_ha <- rowAnnotation(foo = anno_block(gp = gpar(fill = 1:k.number),
#        labels = paste0("C",1:k.number), 
#        labels_gp = gpar(col = "white", fontsize = 8)))


posVec <- as.vector(NULL, mode = "integer")


#right_ha = rowAnnotation(Genes = anno_mark(at = posVec, labels = names(posVec)))
# mHmPlot <- mHmBase
# posVec <- as.vector(NULL, mode = "integer")
# for (i in 1:nrow(dfAnnoHL)){
#   posVec <- c(
#     posVec,
#     grep(as.vector(dfAnnoHL[i, Obio@parameterList$primaryAlignmentGeneID]), as.vector(row.names(mHmPlot)))
#   )
# }
...
figLegend <- paste0(
    "**Figure ",
    figureCount,
    ":** Heatmap showing the ",Obio@parameterList$NtopGenes," most variable genes in the time course. ",
...
for (i in 1:nrow(df.gmt)){
    ...
    dfTemp <- data.frame(
        Correlation_Cluster =  rep(
            paste0(
                '<p style="background-color:',catColor,';text-align:center">',gsub("_", " ", nameTag),'</p>'), 
            length(tempVec)
        ),
...
setGeneric(
    ...
    dfDesign[["sample.group.only"]] <- sapply(dfDesign$sample.group, function(x) unlist(strsplit(x, "_"))[2])
    
    ## Create one plot per category ##
    #setwd(localWorkDir)
    # if ((!make.plots.individually) & make.plots){
    #     pdf(paste0(obj@parameterList$project_id, file.name.suffix,".correlation.plot.profiles.pdf"))
    #     dimensions <- ceiling(sqrt(nrow(df.gmt)))
    #     par.mfrow.default <- par()$mfrow
    #     par.cex.main.default <- par()$cex.main
    #     par(mfrow = c(dimensions, dimensions))
    #     par(cex.main = headline.font.size)
    # }
    ...
    for (i in 1:nrow(df.gmt)) {
        ...
        ## Subset data table ##
        df.sel <- unique(df.sel[df.sel[,obj@parameterList$geneIDcolumn] %in% gene.vector, sel.vec])
        ...
        for (j in 1:length(sample.group.vec)) {
            df.temp[[paste0(sample.group.vec[j], "_median")]] <- apply(df.temp[,grep(paste0(sample.group.vec[j]) ,names(df.temp))],
                                                                           1,
                                                                           median)
        }
        ...
        for (m in 1:length(group.list)) {
            ...
            ## Make ggplot ##
                ...
                plotList[[tag]] <- plotList[[tag]] + geom_line(data=df.temp.plot, aes(x=timepoint, y=y), col=lCol,se=FALSE, span=spanPar) + geom_point(data=df.temp.plot, aes(x=timepoint, y=y), fill=lCol, shape=21)
...
## dfTPM requires gene name col ##
#dfDesign$sample.group <- gsub("MEF$", "", dfDesign$sample.group)
#dfDesign$sample.group <- gsub("Gx$", "", dfDesign$sample.group)
library(scales)
