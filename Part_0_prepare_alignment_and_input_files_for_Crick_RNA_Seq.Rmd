...
FN <- "/camp/stp/babs/working/boeings/Projects/lovellbadger/michelle.neumann/492_bulkRNAseq_mouse_Foxl2_RN20202/data/github.RN20202.basedesign.file.csv"

dfbase <- readr::read_csv(FN)

## In this case, only one of the two sequencing runs, the on on flow-cell 220624_A01366_0228_AHCGKFDMXY was successful.
dfSettings <- dfbase
dfSettings[["sampleID"]] <- sapply(dfSettings$fastq_1, function(x) unlist(strsplit(x, ","))[1])
dfSettings[["sampleID"]] <- sapply(dfSettings$sampleID, function(x) unlist(strsplit(x, "fastq/"))[2])
dfSettings[["sampleID"]] <- as.vector(sapply(dfSettings$sampleID, function(x) unlist(strsplit(x, "_"))[1]))
dfSettings <- unique(dfSettings[,c("sample", "sampleID")])

## Get available files
seqDir <-  c(
    "/camp/stp/sequencing/outputs/babs/data/lovell-badger/michelle.neumann/RN20202/primary_data/220624_A01366_0228_AHCGKFDMXY/fastq/",
    "/camp/stp/sequencing/outputs/babs/data/lovell-badger/michelle.neumann/RN20202/primary_data/220622_A01366_0226_BH55HFDRX2/fastq/"
)
...
## Let's adjust the sample names and make them more meaningful
## load asf the asf sample sheet
FN <- "/camp/stp/babs/working/boeings/Projects/lovellbadger/michelle.neumann/492_bulkRNAseq_mouse_Foxl2_RN20202/data/project-NEU1554-asf-sample-sheet.kapa.only.txt"
dfSample <- read.delim(
    FN,
    sep = "\t",
    stringsAsFactors = F
...
dfSampleUpdate <- unique(dfSample[,c("sample.id", "original.sample.id")])

dfRes <- dplyr::full_join(
    dfRes,
    dfSampleUpdate,
    by = c("sample" = "original.sample.id")
)

dfRes$sample <- dfRes$sample.id
dfRes$sample.id <- NULL
...
readr::write_csv(
    dfRes, 
    path = "/camp/stp/babs/working/boeings/Projects/lovellbadger/michelle.neumann/492_bulkRNAseq_mouse_Foxl2_RN20202/scripts/bulkRNAseq_workflow/design/nf.core.settings.file.csv"
)
...
biologicSeqTools2::createbulkRNASeqAnalysisNFcoreScript(
    ...
    project_id = "RN20202",
    scriptOutdir = "/camp/stp/babs/working/boeings/Projects/lovellbadger/michelle.neumann/492_bulkRNAseq_mouse_Foxl2_RN20202/scripts/",
    NFcoreSettingsFN = "/camp/stp/babs/working/boeings/Projects/lovellbadger/michelle.neumann/492_bulkRNAseq_mouse_Foxl2_RN20202/scripts/bulkRNAseq_workflow/design/nf.core.settings.file.csv",
...
baseDesignFN <- "/camp/stp/babs/working/boeings/Projects/lovellbadger/michelle.neumann/492_bulkRNAseq_mouse_Foxl2_RN20202/data/project-NEU1554-asf-sample-sheet.kapa.only.txt"
