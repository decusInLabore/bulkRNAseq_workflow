...
# ## Get available files
# seqDir <-  c(
#     "/camp/stp/sequencing/outputs/babs/data/liv/colin.hutton/RN22309/primary_data/221125_A01366_0323_BHGG7KDMXY/fastq/",
#     "/camp/stp/sequencing/outputs/babs/data/liv/colin.hutton/RN22309/primary_data/221202_A01366_0326_AHHTTWDMXY/fastq/"
# )
# 
# 
...
## Create base design
dfTemp <- read.csv(
    "/camp/stp/babs/working/eastp/analysis/124_GCSF-macrophages_response_to_yeast-H3_ts245.tessa.swanton/experiments/RN22122/ts245/babs/biologic/inputs/analyse_x_GRCm38/experiment_table.csv"
)

names(dfTemp) <- gsub("^ID$", "sampleID", names(dfTemp))


dfTemp[["sample.id"]] <- paste0(
    dfTemp$genotype, dfTemp$treatment, "_",
    dfTemp$time_point, "_", 
    dfTemp$mouse
)

## create numeric timepoint column
dfTemp[["timepoint"]] <- dfTemp$time_point
dfTemp$timepoint <- as.numeric(gsub("h", "", dfTemp$timepoint))

## Add comparisons ##
dfComp <- read.csv(
    "/camp/stp/babs/working/eastp/analysis/124_GCSF-macrophages_response_to_yeast-H3_ts245.tessa.swanton/experiments/RN22122/ts245/babs/biologic/inputs/analyse_x_GRCm38/contrasts.csv"
)


## Reorder treatment
p1 <- sapply(dfComp$treatment, function(x) {unlist(strsplit(x, "_"))[1]})

p2 <- sapply(dfComp$treatment, function(x) {unlist(strsplit(x, "_"))[2]})

p3 <- sapply(dfComp$treatment, function(x) {unlist(strsplit(x, "h_"))[2]})

dfComp[["teatment_reordered"]] <- paste0(p3, "_", p2, "_", p1)

# Reorder control
pc1 <- sapply(dfComp$control, function(x) {unlist(strsplit(x, "_"))[1]})

pc2 <- sapply(dfComp$control, function(x) {unlist(strsplit(x, "_"))[2]})

pc3 <- sapply(dfComp$control, function(x) {unlist(strsplit(x, "h_"))[2]})

dfComp[["control_reordered"]] <- paste0(pc3, "_", pc2, "_", pc1)


dfTemp[["sample_group_label"]] <- gsub('(.*)_\\w+', '\\1', dfTemp$sample_label)
...
for (i in 1:nrow(dfComp)){
    ...
    tString <- dfComp[i, "teatment_reordered"]
    
    dfTemp[dfTemp$sample_group_label == tString, comp] <- paste0("1_", tString)
    
    cString <- dfComp[i, "control_reordered"]
    
    dfTemp[dfTemp$sample_group_label == cString, comp] <- paste0("2_", cString)
}

## Add LRT #
dfTemp[["LRT_genotype"]] <- dfTemp$genotype
dfTemp[["LRT_time_point"]] <- dfTemp$time_point
dfTemp[["LRT_treatment"]] <- dfTemp$treatment 
...
## Special location in this case:
countTableFN <- "/camp/stp/babs/working/eastp/analysis/124_GCSF-macrophages_response_to_yeast-H3_ts245.tessa.swanton/experiments/RN22122/ts245/babs/nfcore/results/GRCm38/star_rsem/rsem.merged.gene_counts.tsv"
...
TpmTableFN = "/camp/stp/babs/working/eastp/analysis/124_GCSF-macrophages_response_to_yeast-H3_ts245.tessa.swanton/experiments/RN22122/ts245/babs/nfcore/results/GRCm38/star_rsem/rsem.merged.gene_tpm.tsv"
